...
**Handling**:
1. **Check Inception Artifacts**: Verify explicit requirement coverage in inception artifacts
2. **Classify Request**:
   - SPEC_CHANGE: not documented, or documented+implemented but expectation mismatch
   - BUG_FIX: documented behavior but implementation mismatch
3. **Route to Mandatory Path**: Execute SPEC_CHANGE or BUG_FIX path requirements
4. **Log Classification**: Record decision in `aidlc-docs/audit.md` and `aidlc-docs/inception/requirements/change-classification.md`
