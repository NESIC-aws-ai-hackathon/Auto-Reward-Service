...
- If any unit includes BUG_FIX classification, **regression testing is MANDATORY**
...
### Regression Tests (For BUG_FIX)
Create `aidlc-docs/construction/build-and-test/regression-test-instructions.md`:
- Include fixed behavior test coverage (MUST)
- Include directly dependent units based on dependency graph (MUST)
- Include previously passing tests for modified unit (SHOULD)
...
**Validation for BUG_FIX work**:
- Do not mark Build and Test complete until regression tests pass and evidence is recorded
